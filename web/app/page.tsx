...
export default function Home() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-8 sm:py-12">
      <header className="grid gap-5 border-b-[3px] border-ink pb-8 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <p className="mb-3 text-sm font-bold uppercase text-accent">
            MULTIMODAL PRODUCT DISCOVERY // 001
          </p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {DESCRIBE_KEYS.map((key) =>
                      description[key] ? (
                        <span
                          key={key}
                          className="border-[3px] border-ink px-2 py-1 text-xs font-bold uppercase"
                        >
                          {key}: {description[key]}
                        </span>
                      ) : null,
      ...
      {error ? (
        <div className="my-8 border-[3px] border-ink bg-accent p-5 font-bold uppercase">
          ERROR // {error}
        </div>
      ) : null}
      {loading ? (
        <div className="my-10 border-[3px] border-ink bg-ink p-8 text-center text-3xl font-bold uppercase text-white shadow-brut-accent">
          LOADING…
        </div>
      ) : null}
      ...
      {hasSearched ? (
        <form
          ...
          className="grid gap-3 border-t-[3px] border-ink pt-8 md:grid-cols-[auto_1fr_auto] md:items-center"
        >
      ...
      {selectedProduct ? (
        <ProductModal
          ...
          onFindSimilar={showSimilarResults}
        />
      ) : null}
    </div>
  );
