...
export async function POST(req: NextRequest) {
  const { query, colour = null, category = null, style = null, material = null, shape = null } = await req.json();
  ...
  await supabase.from("searches").insert({ query, search_type: "text",
    filters: { colour, category, style, material, shape }, num_results: data.length });
